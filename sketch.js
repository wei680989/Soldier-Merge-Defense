...
const MONSTER_ELITE_SPAWN_X_OFFSET = 40;
...
const MAX_SOLDIERS_ALLOWED = 20;

// --- Game State ---
let gold; let currentLevel; let gamePhase;
let soldiers = []; let monsters = []; let draggedSoldier = null;
//let monstersToSpawn = []; // Removed - monsters created directly
let nextWavePreview = [];
let nextWaveSpawnList = [];
let spawnTimer = 0; let spawnInterval = 85; // Keep interval for potential future use? Or remove spawnTimer too. For now, keep interval.
...
// --- Asset Definitions --- (Balanced Version)
const BASE_SOLDIER_TYPES = { axe: { name: "斧兵", cost: 6, baseHealth: 70, baseDamage: 9, attackSpeed: 50, range: 70, attackDistance: UNIT_RADIUS * 1.6, speed: 1.0, color: [0, 150, 0], upgradeMultiplier: 1.8, isMelee: true }, spear: { name: "長矛兵", cost: 8, baseHealth: 65, baseDamage: 8, attackSpeed: 58, range: 90, attackDistance: UNIT_RADIUS * 2.5, speed: 0.9, color: [100, 100, 0], upgradeMultiplier: 1.85, isMelee: true }, archer: { name: "弓箭手", cost: 10, baseHealth: 40, baseDamage: 7, attackSpeed: 60, range: 200, attackDistance: 200, speed: 0.7, color: [0, 0, 200], upgradeMultiplier: 1.95, isMelee: false }, tank: { name: "坦克", cost: 14, baseHealth: 180, baseDamage: 6, attackSpeed: 75, range: 60, attackDistance: UNIT_RADIUS * 1.8, speed: 0.5, color: [100, 100, 120], upgradeMultiplier: 1.6, isMelee: true } };
const ADVANCED_SOLDIER_TYPES = { berserker: { name: "狂戰士", unlocked: false, cost: 25, buyLimit: ADVANCED_UNIT_BUY_LIMIT, buyCount: 0, baseHealth: 220, baseDamage: 38, attackSpeed: 30, range: 80, attackDistance: UNIT_RADIUS * 1.8, speed: 1.2, color: [255, 50, 50], upgradeMultiplier: 1.9, isMelee: true }, headhunter: { name: "獵頭者", unlocked: false, cost: 28, buyLimit: ADVANCED_UNIT_BUY_LIMIT, buyCount: 0, baseHealth: 180, baseDamage: 30, attackSpeed: 35, range: 75, attackDistance: UNIT_RADIUS * 1.7, speed: 1.5, color: [0, 200, 100], upgradeMultiplier: 2.0, isMelee: true }, juggernaut: { name: "重裝兵", unlocked: false, cost: 40, buyLimit: ADVANCED_UNIT_BUY_LIMIT, buyCount: 0, baseHealth: 700, baseDamage: 25, attackSpeed: 70, range: 70, attackDistance: UNIT_RADIUS * 2.0, speed: 0.4, color: [80, 80, 80], upgradeMultiplier: 1.8, isMelee: true }, marksman: { name: "神射手", unlocked: false, cost: 35, buyLimit: ADVANCED_UNIT_BUY_LIMIT, buyCount: 0, baseHealth: 120, baseDamage: 28, attackSpeed: 48, range: 320, attackDistance: 320, speed: 0.7, color: [100, 0, 255], upgradeMultiplier: 2.3, isMelee: false }, phalanx: { name: "方陣兵", unlocked: false, cost: 32, buyLimit: ADVANCED_UNIT_BUY_LIMIT, buyCount: 0, baseHealth: 400, baseDamage: 22, attackSpeed: 55, range: 100, attackDistance: UNIT_RADIUS * 2.8, speed: 0.6, color: [180, 140, 0], upgradeMultiplier: 2.0, isMelee: true }, bulwarkArcher: { name: "堡壘射手", unlocked: false, cost: 30, buyLimit: ADVANCED_UNIT_BUY_LIMIT, buyCount: 0, baseHealth: 220, baseDamage: 18, attackSpeed: 58, range: 230, attackDistance: 210, speed: 0.5, color: [50, 100, 180], upgradeMultiplier: 2.0, isMelee: false } };
...
const MONSTER_TYPES = { goblin: { name: "哥布林", baseHealth: 60, baseDamage: 9, attackSpeed: 55, range: 60, attackDistance: UNIT_RADIUS * 1.6, speed: 0.8, reward: 3, color: [200, 0, 0], isMelee: true }, goblinSpear: { name: "投矛哥布林", baseHealth: 50, baseDamage: 8, attackSpeed: 68, range: 150, attackDistance: 150, speed: 0.7, reward: 4, color: [220, 50, 0], isMelee: false }, ogre: { name: "食人魔", baseHealth: 280, baseDamage: 28, attackSpeed: 75, range: 80, attackDistance: UNIT_RADIUS * 2.0, speed: 0.4, reward: 8, color: [100, 50, 0], isMelee: true }, skeletonArcher: { name: "骷髏弓手", baseHealth: 95, baseDamage: 18, attackSpeed: 55, range: 190, attackDistance: 180, speed: 0.6, reward: 7, color: [180, 180, 180], isMelee: false }, boar: { name: "野豬", baseHealth: 180, baseDamage: 22, attackSpeed: 48, range: 50, attackDistance: UNIT_RADIUS * 1.5, speed: 1.5, reward: 6, color: [139, 69, 19], isMelee: true }, eliteGoblin: { name: "精英哥布林", baseHealth: 200, baseDamage: 30, attackSpeed: 35, range: 70, attackDistance: UNIT_RADIUS * 1.7, speed: 1.4, reward: 10, color: [255, 100, 100], isMelee: true }, eliteOgre: { name: "精英食人魔", baseHealth: 550, baseDamage: 45, attackSpeed: 65, range: 90, attackDistance: UNIT_RADIUS * 2.2, speed: 0.5, reward: 20, color: [150, 75, 0], isMelee: true }, eliteSkeletonArcher: { name: "精英骷髏弓手", baseHealth: 200, baseDamage: 35, attackSpeed: 45, range: 250, attackDistance: 250, speed: 0.8, reward: 18, color: [210, 210, 210], isMelee: false }, eliteKnight: { name: "精英騎士", baseHealth: 500, baseDamage: 40, attackSpeed: 48, range: 90, attackDistance: UNIT_RADIUS * 2.1, speed: 1.1, reward: 30, color: [192, 192, 192], isMelee: true }, miniBossOrc: { name: "獸人隊長", baseHealth: 1200, baseDamage: 65, attackSpeed: 58, range: 100, attackDistance: UNIT_RADIUS * 2.5, speed: 0.6, reward: 60, color: [0, 100, 0], isMelee: true }, bossHydra: { name: "九頭蛇", baseHealth: 1500, baseDamage: 60, attackSpeed: 42, range: 150, attackDistance: UNIT_RADIUS * 3.0, speed: 0.5, reward: 90, color: [0, 150, 150], isMelee: true }, finalBossDragon: { name: "巨龍", baseHealth: 3000, baseDamage: 100, attackSpeed: 70, range: 350, attackDistance: 300, speed: 0.5, reward: 250, color: [50, 50, 50], isMelee: false } };
...
function draw() {
    ...
    switch (gamePhase) {
        case 'startScreen':
            ...
            break;
        case 'questSelection':
            drawQuestSelectionScreen();
            break;
        case 'buy':
            drawBattlefield(); drawUI(); drawPlayerArmyStats(); drawEnemyPreview();
            handleDragging(); drawSoldiers(); drawMonsters(); // Draw placeholders if needed
            detectHover(); displayGameMessage(); updateBuyButtons();
            break;
        case 'fight':
            drawBattlefield(); drawUI(); drawPlayerArmyStats();
            handleDragging(); // Should be inactive
            drawSoldiers(); drawMonsters();
            detectHover(); displayGameMessage();
            // --- Core fight logic ---
            updateUnits();
            handleCombat();
            // createMonstersForWave(); // <<<--- REMOVE THIS CALL FROM DRAW LOOP ---<<<
            // spawnMonsters(); // <<<--- REMOVE THIS CALL FROM DRAW LOOP (Old name) ---<<<
            checkWaveEnd();
            checkGameOver();
            break; // End fight case
        case 'restarting':
            drawBattlefield(); drawUI(); drawSoldiers(); drawMonsters(); displayGameMessage();
            break;
        case 'win':
            drawBattlefield(); drawUI(); drawSoldiers(); drawMonsters(); drawVictoryScreen();
            break;
        default:
            console.error("Unknown phase:", gamePhase); initializeGame();
    }

    // Draw tooltip last
    if (hoveredUnitInfo && gamePhase !== 'startScreen' && gamePhase !== 'questSelection') {
        drawUnitTooltip(hoveredUnitInfo.unit, hoveredUnitInfo.type, mouseX, mouseY);
    }
} // End draw()
...
// --- NEW: Function to draw player army stats ---
function drawPlayerArmyStats() {
    push();
    let infoXLeft = 10; // Starting X for the stats block
    let statsY = 40;    // Starting Y for the stats block
    let statsLineHeight = 14;
    let soldierEntryWidth = 120; // Estimated width needed per soldier entry
    let soldierEntrySpacing = 5; // Horizontal space between entries
    let maxPerRow = 6;           // Max soldiers per row
    let soldierCount = soldiers.length;

    if (soldierCount === 0 && gamePhase !== 'buy') return; // Don't draw if no soldiers unless in buy phase

    // --- Calculate background dimensions ---
    let numRows = ceil(soldierCount / maxPerRow) || 1; // At least 1 row high for title
    let bgHeight = 20 + (numRows * statsLineHeight) + 5; // Height based on rows
    let bgWidth = (maxPerRow * (soldierEntryWidth + soldierEntrySpacing)) - soldierEntrySpacing + 10; // Width based on max items + padding
    bgWidth = min(bgWidth, width - 20); // Limit width to canvas bounds


    // Draw background
    fill(0, 0, 50, 180);
    noStroke();
    rect(infoXLeft - 5, statsY - 5, bgWidth, bgHeight, 5);

    // Draw Title
    fill(255);
    textSize(12);
    textAlign(LEFT, TOP);
    text(`我方士兵 (${soldierCount}/${MAX_SOLDIERS_ALLOWED}):`, infoXLeft, statsY);
    statsY += statsLineHeight + 3; // Move down for the first row

    // --- Draw Soldier List ---
    textSize(10);
    let currentX = infoXLeft;
    let soldiersInRow = 0;

    soldiers.forEach(s => {
        if (s) {
            const definition = getUnitDefinition(s.type);
            const name = definition ? definition.name : s.type;
            const levelPrefix = ADVANCED_SOLDIER_TYPES[s.type] ? "★" : "L";

            // Prepare text content
            let soldierText = `- ${name} (${levelPrefix}${s.level})`;

            // Check if current entry fits in the row width, or if row is full
             // Estimate text width - might need adjustment based on font/actual rendering
             // let estimatedTextWidth = textWidth(soldierText) + 10;
            if (soldiersInRow >= maxPerRow /*|| currentX + estimatedTextWidth > infoXLeft + bgWidth - 10 */) {
                currentX = infoXLeft;       // Move to next row, reset X
                statsY += statsLineHeight; // Move Y down
                soldiersInRow = 0;         // Reset row counter
            }

            // Draw soldier info at current position
            text(soldierText, currentX, statsY);

            // Move X for the next entry in the same row
            currentX += soldierEntryWidth + soldierEntrySpacing;
            soldiersInRow++;
        }
    });
    pop();
...
function buySoldier(type) { if (gamePhase !== 'buy') return; if (soldiers.length >= MAX_SOLDIERS_ALLOWED) { gameMessage = `士兵數量已達上限 (${MAX_SOLDIERS_ALLOWED})！`; messageTimer = 120; console.log("Cannot buy soldier: Limit reached."); return; } const stats = getUnitDefinition(type); if (!stats) { console.error("Invalid soldier type:", type); return; } const cost = stats.cost || 0; const isAdv = ADVANCED_SOLDIER_TYPES[type]; if (isAdv) { if (!stats.unlocked) { console.log("Cannot buy locked advanced unit:", type); return; } if (stats.buyCount >= stats.buyLimit) { console.log(`Buy limit reached for ${stats.name}`); gameMessage = `${stats.name} 已達購買上限!`; messageTimer = 90; return; } } if (gold >= cost) { gold -= cost; if (isAdv) { stats.buyCount++; } const newSoldier = { type: type, level: 1, x: SOLDIER_SPAWN_X + random(-15, 15), y: random(BATTLEFIELD_Y_START + UNIT_RADIUS + 10, BATTLEFIELD_Y_START + BATTLEFIELD_HEIGHT - UNIT_RADIUS - 10), maxHealth: stats.baseHealth, health: stats.baseHealth, damage: stats.baseDamage, attackSpeed: stats.attackSpeed, range: stats.range, attackDistance: stats.attackDistance, speed: stats.speed, lastAttackFrame: -Infinity, target: null, isMoving: false, color: stats.color.slice(), isMelee: stats.isMelee, lastBuyPhaseX: 0, lastBuyPhaseY: 0 }; newSoldier.lastBuyPhaseX = newSoldier.x; newSoldier.lastBuyPhaseY = newSoldier.y; soldiers.push(newSoldier); console.log(`Bought ${stats.name}. Gold left: ${gold}`); updateBuyButtons(); updateQuestProgress('reachGold', null, gold); } else { console.log(`Not enough gold to buy ${stats.name}`); gameMessage = `金幣不足 (${cost} G)`; messageTimer = 90;} }
// --- Modified startWave to use createMonstersForWave ---
function startWave() {
    if (gamePhase !== 'buy') { console.log("Cannot start wave, not in buy phase."); return; }
    console.log(`Attempting to start wave ${currentLevel}. Current phase: ${gamePhase}`);

    // Use the pre-calculated spawn list stored in nextWaveSpawnList
    console.log("Using pre-calculated spawn list:", nextWaveSpawnList);
     if (!nextWaveSpawnList || !Array.isArray(nextWaveSpawnList) || nextWaveSpawnList.length === 0) {
         console.error("nextWaveSpawnList is empty or invalid when trying to start wave! Forcing calculation for current level.");
         calculateNextWavePreview(currentLevel); // Attempt recovery
         if (!nextWaveSpawnList || nextWaveSpawnList.length === 0){
            console.error("Recalculation failed! Using fallback.");
            nextWaveSpawnList = ['goblin'];
         }
     }

    gamePhase = 'fight';
    waveStartTime = millis();
    monsters = []; // Clear monsters from previous wave (should be empty anyway)

    // Create monsters based on the ensured nextWaveSpawnList
    createMonstersForWave(nextWaveSpawnList); // <<< Create monsters HERE, only once

    gameMessage = "";
    console.log("Setting soldiers to moving state.");
    soldiers.forEach(s => { if (s) { s.lastBuyPhaseX = s.x; s.lastBuyPhaseY = s.y; s.isMoving = true; s.target = null; s.lastAttackFrame = -Infinity; } });

    console.log("Calculating PREVIEW for next wave (level", currentLevel + 1 + ")");
    // Calculate preview (and store actual list) for the *next* wave AFTER this one
    calculateNextWavePreview(currentLevel + 1);

    console.log("Updating buttons for fight phase.");
    updateBuyButtons(); updateStartButton();
    console.log("startWave function finished. New phase:", gamePhase);
}
function calculateNextWavePreview(level) { if (level > MAX_LEVEL) { nextWavePreview = [{ type: "已通關", count: "!" }]; nextWaveSpawnList = []; return; } console.log(`Calculating preview AND spawn list for level ${level}`); const spawnListResult = generateMonsterWave(level); if (!spawnListResult || !Array.isArray(spawnListResult)) { console.error("generateMonsterWave returned invalid data!", spawnListResult); nextWaveSpawnList = ['goblin']; } else { nextWaveSpawnList = spawnListResult; } let counts = {}; nextWaveSpawnList.forEach(type => { counts[type] = (counts[type] || 0) + 1; }); let composition = []; for (const type in counts) { if (counts.hasOwnProperty(type)){ composition.push({ type: type, count: counts[type] }); } } nextWavePreview = composition; console.log(`Preview L${level}:`, nextWavePreview); }
// --- Modified generateMonsterWave with L1/L2 adjustments & balancing ---
function generateMonsterWave(level) {
    let waveComposition = []; let spawnList = []; let basePoints = 0; let tempSpawnList = [];
    const regularPool = ['goblin', 'goblinSpear', 'ogre', 'skeletonArcher', 'boar'];
    const weakRegularPool = ['goblin', 'goblinSpear'];
    const strongRegularPool = ['ogre', 'boar'];
    ...
    else if (level >= 4 && level <= 6) { basePoints = 12 + level * 5; let eliteType = random(elitePool); let eliteCount = (level >= 5) ? floor(random(2, 3)) : floor(random(1, 3)); for(let i=0; i<eliteCount; i++) tempSpawnList.push(eliteType); basePoints -= (costs[eliteType] || 10) * eliteCount; while (basePoints > 0 && tempSpawnList.length < 30) { let available = regularPool.filter(type => costs[type] <= basePoints); if (available.length === 0) break; let type = random(available); tempSpawnList.push(type); basePoints -= costs[type]; } }
    else if (level === 3) { basePoints = 20 + level * 5; let eliteType = random(elitePool); tempSpawnList.push(eliteType); basePoints -= (costs[eliteType] || 10); let strongType = random(strongRegularPool); let strongCount = 3; for (let i = 0; i < strongCount; i++) { if (basePoints >= (costs[strongType] || 5)) { tempSpawnList.push(strongType); basePoints -= (costs[strongType] || 5); } else { break; } } while (basePoints > 0 && tempSpawnList.length < 25) { let available = weakRegularPool.filter(type => costs[type] <= basePoints); if (available.length === 0) break; let type = random(available); tempSpawnList.push(type); basePoints -= costs[type]; } }
    else { // Levels 1 & 2
        if (level === 1) { basePoints = 12; let level1Pool = ['goblin', 'goblinSpear']; let level1Costs = { goblin: 1, goblinSpear: 1.5 }; while (basePoints > 0 && tempSpawnList.length < 8) { let available = level1Pool.filter(type => level1Costs[type] <= basePoints); if (available.length === 0) break; let type = random(available); tempSpawnList.push(type); basePoints -= level1Costs[type]; } if (tempSpawnList.length < 4) tempSpawnList.push('goblin'); }
        else { basePoints = 25; let level2Pool = ['goblin', 'goblinSpear', 'boar']; let level2Costs = { goblin: 1, goblinSpear: 1.5, boar: 3 }; if (random() < 0.3) { let strongType = 'boar'; if (basePoints >= (level2Costs[strongType] || 3)) { tempSpawnList.push(strongType); basePoints -= (level2Costs[strongType] || 3); } } while (basePoints > 0 && tempSpawnList.length < 15) { let available = ['goblin', 'goblinSpear'].filter(type => level2Costs[type] <= basePoints); if (available.length === 0) break; let type = random(available); tempSpawnList.push(type); basePoints -= level2Costs[type]; } if (tempSpawnList.length < 6) tempSpawnList.push('goblin'); }
    }

    if ([10, 9].includes(level)) { spawnList = []; waveComposition.forEach(item => { for (let i = 0; i < item.count; i++) spawnList.push(item.type); }); }
    else { spawnList = tempSpawnList; }
    ...
    // Calculate composition based on the final list for preview update
    let finalCounts = {}; spawnList.forEach(type => { finalCounts[type] = (finalCounts[type] || 0) + 1; });
    waveComposition = []; for (const type in finalCounts) { if(finalCounts.hasOwnProperty(type)) waveComposition.push({ type: type, count: finalCounts[type] }); }
    // The caller (calculateNextWavePreview) will handle updating nextWavePreview

    return spawnList;
}
// --- Modified createMonstersForWave ---
function createMonstersForWave(spawnList) {
    console.log("Creating monsters for wave with list:", spawnList);
    monsters = []; // Clear existing monsters immediately
    if (!spawnList || !Array.isArray(spawnList) || spawnList.length === 0) {
        console.error("createMonstersForWave called with empty or invalid spawnList! Cannot create monsters.");
        return; // Stop execution if list is bad
    }

    spawnList.forEach(monsterTypeKey => {
        const stats = MONSTER_TYPES[monsterTypeKey];
        if (!stats) { console.error("Invalid monster type in spawnList:", monsterTypeKey); return; }
        const healthMultiplier = 1 + (currentLevel - 1) * 0.18; // Increased HP scaling
        const damageMultiplier = 1 + (currentLevel - 1) * 0.15; // Increased Dmg scaling
        let spawnX;
        if (monsterTypeKey.includes('Boss') || monsterTypeKey.includes('Elite')) { spawnX = width - MONSTER_ELITE_SPAWN_X_OFFSET + random(-10, 10); }
        else { spawnX = width - MONSTER_SPAWN_X_OFFSET + random(-15, 15); }
        monsters.push({ type: monsterTypeKey, x: spawnX, y: random(BATTLEFIELD_Y_START + UNIT_RADIUS + 10, BATTLEFIELD_Y_START + BATTLEFIELD_HEIGHT - UNIT_RADIUS - 10), maxHealth: (stats.baseHealth || 100) * healthMultiplier, health: (stats.baseHealth || 100) * healthMultiplier, damage: (stats.baseDamage || 5) * damageMultiplier, attackSpeed: stats.attackSpeed, range: stats.range, attackDistance: stats.attackDistance, speed: stats.speed, reward: stats.reward || 1, lastAttackFrame: -Infinity, target: null, isMoving: true, color: stats.color ? stats.color.slice() : [128, 128, 128], isMelee: stats.isMelee });
    });
    console.log("Successfully created monsters:", monsters.length);
}
...
function checkWaveEnd() {
    // Check if monsters array exists and is empty.
    if (gamePhase === 'fight' && monsters && monsters.length === 0 ) {
        ...
        if (currentWaveCleared >= MAX_LEVEL) {
            ...
            gamePhase = 'buy';
            let waveReward = 10 + currentLevel * 4; // Increased wave clear gold reward
            gold += waveReward;
            currentLevel++;
...
function checkGameOver() { if (gamePhase === 'fight' && soldiers.length === 0 && monsters.length > 0 ) { console.log("Game Over - All soldiers defeated"); gamePhase = 'restarting'; gameMessage = "遊戲結束 - 你失敗了"; messageTimer = 120; hideGameButtons(); updateBuyButtons(); updateStartButton(); } }
...
function unlockAdvancedUnits() {
    ...
    createGameButtons();
    if (gamePhase === 'buy' || gamePhase === 'questSelection') { showGameButtons(); }
